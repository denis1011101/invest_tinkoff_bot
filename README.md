...
### Risk management
- **Dynamic position size** — the order size in lots follows the market regime instead of a fixed `LOTS_PER_ORDER`: `LOTS_MIN` (default `1`) in SIDE/DOWN, `LOTS_UP` (default `2`) in an UP trend, and `LOTS_MAX` (default `3`) when the index has grown by at least `LOTS_UP_STRONG_PCT` (default `2%`) over the last `LOTS_INDEX_LOOKBACK` closed daily closes. The growth needs the **full** window: a shorter index history yields no growth figure at all and UP falls back to the middle size, so a three-day move is never read as a five-day one. The signal can only raise the size; three hard ceilings cut it back — the lot count (`LOTS_MAX`), the rouble cost of a single order (`MAX_LOT_RUB`) and the actually available cash. An instrument that does not fit even with one lot is skipped (`stage=sizing` in the BUY funnel). In the UP path a shortage of cash shrinks the order instead of cancelling it, and the shrink runs **before** the order-value gates (position share, daily budget, equity exposure) — otherwise a three-lot order fails those limits while the one lot the account can actually afford would have fit them. Shadow mode still evaluates the pre-cash size: it answers which signals the bot would take if cash were available. `DYNAMIC_LOTS=0` restores the fixed `LOTS_PER_ORDER` behaviour, cash buffer included. See [`TradingLogic::PositionSizing`](lib/position_sizing.rb).
...
- `MAX_LOT_RUB` — per-order rouble ceiling (`price_per_lot * lots`) used in `Runner` and momentum buy helper. With dynamic sizing it is the main budget knob: it caps a 3-lot order as strictly as a 1-lot one, so set it to the share of the account one position may cost.
...
- `DYNAMIC_LOTS` — `1` (default) sizes each order by market regime; `0` falls back to the fixed `LOTS_PER_ORDER`.
- `LOTS_MIN` — order size in SIDE/DOWN and the floor everywhere (default `1`). Also the size the universe filter assumes: an instrument that does not fit `MAX_LOT_RUB` with `LOTS_MAX` lots is still allowed at `LOTS_MIN`.
- `LOTS_UP` — order size in an UP trend (default `2`).
- `LOTS_MAX` — order size on confident index growth and the hard ceiling for any order (default `3`).
- `LOTS_UP_STRONG_PCT` — index growth over the lookback window that unlocks `LOTS_MAX` (default `0.02` = +2%).
- `LOTS_INDEX_LOOKBACK` — number of closed daily index closes used to measure that growth (default `5`).
- `LOTS_PER_ORDER` — fixed order quantity in lots; used only when `DYNAMIC_LOTS=0` (`quantity = LOTS_PER_ORDER`, shares = `lot_size * LOTS_PER_ORDER`).
