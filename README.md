...
### Risk management
- **Dynamic position size** — the order size in lots follows the market regime instead of a fixed `LOTS_PER_ORDER`: `LOTS_MIN` (default `1`) in SIDE/DOWN, `LOTS_UP` (default `2`) in an UP trend, and `LOTS_MAX` (default `3`) when the index has grown by at least `LOTS_UP_STRONG_PCT` (default `2%`) over the last `LOTS_INDEX_LOOKBACK` closed daily closes. The signal can only raise the size; three hard ceilings cut it back — the lot count (`LOTS_MAX`), the rouble cost of a single order (`MAX_LOT_RUB`) and the actually available cash. An instrument that does not fit even with one lot is skipped (`stage=sizing` in the BUY funnel), and in the UP path a shortage of cash shrinks the order instead of cancelling it — the money gates are monotonic in order value, so the smaller order passes wherever the bigger one already passed. `DYNAMIC_LOTS=0` restores the fixed `LOTS_PER_ORDER` behaviour. See [`TradingLogic::PositionSizing`](lib/position_sizing.rb).
