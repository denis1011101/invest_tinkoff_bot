...
- **Cash reserve guard** — caps the share of the account held in shares (`MAX_SHARES_SHARE`, disabled when unset/`0`), keeping dry powder for future dips. Complements `MAX_POSITION_SHARE`, which limits only one ticker. The denominator is `total_amount_portfolio` (all asset classes, bonds included), falling back to shares + currencies. Unlike `MAX_POSITION_SHARE` this guard is **fail-closed**: if the portfolio cannot be read or its total is non-positive, the BUY is blocked and a warning is logged. Callers that reuse one portfolio snapshot for several candidates must add the already-committed value of that pass to `planned_buy_value` — the UP branch does this via `run_committed`.
...
- **Volume-aware filters and ranking** — optional relative volume filter for entries (`MIN_RELATIVE_VOLUME`) and cross-sectional ranking (`VOLUME_COMPARE_MODE=relative|turnover`). The current day's candle is still accumulating volume, so the ratio is normalized by the elapsed share of the MOEX session (morning, main and evening segments) before it is compared to the threshold — without that, rvol grows mechanically towards the close and any threshold above ~1 is unreachable during the day. Closed days are used as-is. While too little of the session has elapsed to judge (below ~5% of expected daily volume — crossed at 07:12 UTC, so with a 5-minute cron the first run that can buy is 07:15 UTC), the ratio is treated as unreliable and the BUY is blocked rather than allowed — early in the morning session a single large trade would otherwise look like a volume spike.
