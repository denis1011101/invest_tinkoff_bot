...
module TradingLogic
  module Utils
    ...
    # Сессии MOEX по акциям в UTC с долей дневного объёма на каждую:
    #   утренняя  06:50–09:50 MSK = 03:50–06:50 UTC
    #   основная  10:00–18:45 MSK = 07:00–15:45 UTC
    #   вечерняя  19:05–23:50 MSK = 16:05–20:50 UTC
    # Веса — грубая оценка распределения оборота (утро ~3%, вечер ~10%); внутри сессии
    # считаем объём равномерным. Модель приблизительная, но снимает главное искажение —
    # сравнение неполного дня с полными. Пропущенная утренняя сессия — это не просто
    # неточность: дневная свеча уже содержит утренний объём, и без сегмента он делился
    # бы на пол MIN_SESSION_FRACTION, раздувая rvol в разы.
    SESSION_SEGMENTS = [
      { from: (3 * 60) + 50, to: (6 * 60) + 50, weight: 0.03 },
      { from: 7 * 60, to: (15 * 60) + 45, weight: 0.87 },
      { from: (16 * 60) + 5, to: (20 * 60) + 50, weight: 0.10 }
    ].freeze
