...
RSpec.describe TradingLogic::StrategyLogSummary do
  ...
  let(:no_sell_orders) { { sell_orders: {}, sell_mismatch_count: 0 } }
  ...
  it 'counts both SELL paths in the half-open window, grouping unresolved instruments by FIGI' do
    ...
    expect(result).to eq(ok: true, unknown_session_count: 2, instrument_unresolved: { 'BBG008F2T3T2' => 2, 'F2' => 1 },
                         invalid_lot: { 'BBG008F2T3T2' => 2 }, **no_sell_orders)
  ...
  it 'ignores unrelated messages, other severities and malformed timestamps' do
    ...
    expect(result).to eq(ok: true, unknown_session_count: 0, instrument_unresolved: {}, invalid_lot: {}, **no_sell_orders)
  ...
  it 'counts actual helper warnings formatted by the production Logger formatter' do
    ...
    expect(summarize(output.string)).to eq(
      ...
      instrument_unresolved: { 'BBG008F2T3T2' => 2 }, invalid_lot: { 'BBG008F2T3T2' => 2 }, **no_sell_orders
    )
  end

  it 'reports SELL order outcomes written by the helpers, once per event and only inside the window' do
    output = StringIO.new
    logger = Logger.new(output)
    logger.formatter = TradingLogic::StrategyLogFormatter.new
    allow(Time).to receive(:now).and_return(from + 60)
    ...
    { 'c1' => 'CNRU', 'p1' => 'TATN' }.each do |id, ticker|
      result = { category: :sent_not_filled, response: OpenStruct.new(order_id: id) }
      helpers.register_sell_order!(state, ticker, figi: "F_#{id}", result: result, lots: 2, reason: 'signal')
    end
    { 'c1' => [:cancelled, 0], 'p1' => [:partially_executed, 1] }.each do |id, (outcome, executed)|
      helpers.finalize_pending_sell!(state, id, state['pending_sells'][id], outcome: outcome, status: cancelled,
                                                                            lots_executed: executed, lots_requested: 2,
                                                                            logger: logger)
    end
    helpers.alert_long_pending_sell!(logger, { 'ticker' => 'VTBR', 'ts' => from.iso8601 }, now: from + 7200)
    stuck = { 'ticker' => 'VTBR', 'broker_order_id' => 's1', 'terminal_confirm_attempts' => 4 }
    helpers.unresolved_pending_sell!(logger, stuck)
    helpers.unresolved_pending_sell!(logger, stuck)
    ...
    expect(described_class.format(result)).to include(
      '⚠️ SELL не исполнена, заявка снята: CNRU (1).',
  ...
  it 'recovers the report window from compressed and plain rotations after copytruncate' do
    Dir.mktmpdir do |dir|
      ...
      expect(result).to eq(ok: true, unknown_session_count: 1,
                           instrument_unresolved: { 'BBG008F2T3T2' => 1 }, invalid_lot: { 'BBG008F2T3T2' => 1 }, **no_sell_orders)
    end
  end
  ...
  it 'does not read old rotations, even a corrupt gzip, while counting the current log' do
    Dir.mktmpdir do |dir|
      ...
      result = described_class.new(path: path).build(from: from, to: to)
      expect(result).to eq(ok: true, unknown_session_count: 1, instrument_unresolved: {}, invalid_lot: {}, **no_sell_orders)
    end
  end
  ...
  it 'reports zero for an empty readable log' do
    expect(summarize('')).to eq(ok: true, unknown_session_count: 0, instrument_unresolved: {}, invalid_lot: {}, **no_sell_orders)
  end
